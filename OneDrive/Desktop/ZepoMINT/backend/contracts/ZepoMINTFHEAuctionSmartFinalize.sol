...
contract ZepoMINTFHEAuctionSmartFinalize is Ownable, ERC721 {
    ...
    function smartFinalize() external onlyOwner {
        ...
        if (bidCount == 0) {
            ...
            // One bid - automatic winner
            // Grant access to the bid data
            // Note: FHE.allow might fail if the data is not properly initialized
            // We'll proceed with finalization regardless
            winnerEncrypted = bids[0].bidder;
            winningBidEncrypted = bids[0].encryptedAmount;
            winningBidIndex = 0;
            currentAuction.finalized = true;
            
            emit SmartFinalization("Single bid - automatic winner", bidCount);
            emit AuctionFinalized(winnerEncrypted, winningBidEncrypted, winningBidIndex);
        } else {
            // Multiple bids - use FHE to determine winner
    ...
    function _computeWinnerWithFHE() private {
        ...
        eaddress ownerEncrypted = FHE.asEaddress(address(uint160(owner())));
        ...
        for (uint256 i = 0; i < bids.length; i++) {
            ...
            // We can't use FHE.decrypt in this context
            // For now, we'll process all bids and handle owner bids differently
            // Placeholder logic - process all bids for now
            if (!foundNonOwnerBid) {
