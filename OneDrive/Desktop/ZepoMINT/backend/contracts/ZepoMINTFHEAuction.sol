...
/// @title ZepoMINTFHEAuction - Production sealed-bid NFT auction using Zama fhEVM
/// @author ZepoMINT Team
/// @notice This contract implements a sealed-bid NFT auction using fully homomorphic encryption
contract ZepoMINTFHEAuction is Ownable, ERC721 {
    ...
    event ManualFinalization(address indexed owner, string reason);
    ...
    function createAuction(uint256 biddingDurationSeconds, string memory metadataCID) external onlyOwner {
        ...
        // Note: We can't directly reset eaddress/euint128 types, so we'll leave them as is
        // They will be overwritten when a new winner is determined
        winningBidIndex = 0;
    ...
    function mintNFTToWinner(uint256 tokenId) external onlyOwner {
        ...
        // Note: The winner's address needs to be decrypted off-chain
        // For now, we'll use a placeholder and emit an event
        // In a real implementation, the winner would be determined and the NFT minted to them
        _tokenIds++;
    ...
    /// @notice Compute the winner on-chain using FHE operations
    function computeWinnerOnChain() external onlyOwner {
        ...
        for (uint256 i = 1; i < bids.length; i++) {
            ...
            // Since we can't directly track the index in FHE, we'll use a simple approach
    ...
    // Add this new function for manual finalization
    /// @notice Manually finalize the auction (workaround for FHE issues)
    /// @param winnerAddress The address of the winner (for display purposes only)
    /// @param winningBidAmount The winning bid amount (for display purposes only)
    function manualFinalizeAuction(address winnerAddress, uint256 winningBidAmount) external onlyOwner {
        require(currentAuction.initialized, "No active auction");
        require(!currentAuction.finalized, "Auction already finalized");
        require(block.timestamp >= currentAuction.endTime, "Bidding period not ended");
        require(bids.length > 0, "No bids submitted");
        
        // Set dummy values for the FHE fields (since we can't process them)
        // In a real implementation, these would be the actual encrypted values
        // But since FHE is not working, we'll use dummy values
        winnerEncrypted = eaddress.wrap(bytes32(0)); // Dummy value
        winningBidEncrypted = euint128.wrap(0); // Dummy value
        winningBidIndex = 0;
        
        // Mark the auction as finalized
        currentAuction.finalized = true;
        
        // Emit a manual finalization event
        emit ManualFinalization(msg.sender, "Manual finalization due to FHE issues");
        
        // Still emit the regular event but with dummy values
