...
contract ZepoMINTFHEAuctionSmartFinalize is Ownable, ERC721 {
    ...
    function smartFinalize() external onlyOwner {
        ...
        if (bidCount == 0) {
            ...
            // One bid - check if it's from owner (prevent owner from winning)
            // eaddress ownerEncrypted = FHE.asEaddress(owner());
            // ebool isOwnerBid = FHE.eq(bids[0].bidder, ownerEncrypted);
            
            // Since we can't decrypt in Solidity, we'll use a simplified approach
            // In practice, this would need to be handled off-chain
            winnerEncrypted = bids[0].bidder;
            winningBidEncrypted = bids[0].encryptedAmount;
            winningBidIndex = 0;
            currentAuction.finalized = true;
            
            emit SmartFinalization("Single bid - assumed non-owner winner", bidCount);
            ...
            _computeWinnerWithFHEExcludingOwner();
        }
    }
    
    /// @notice Private function to compute winner using FHE while excluding owner
    function _computeWinnerWithFHEExcludingOwner() private {
        ...
        for (uint256 i = 0; i < bids.length; i++) {
            ...
            // Since we can't decrypt in Solidity, we'll use a different approach
            // We'll process all bids but track if any are non-owner bids
            if (!foundNonOwnerBid) {
                // First bid (could be owner or non-owner)
                highestBid = bids[i].encryptedAmount;
                winner = bids[i].bidder;
                highestBidIndex = i;
                // We'll assume at least one non-owner bid for now
    ...
    function mintNFTToWinner(uint256 /* tokenId */) external {
        ...
        // In a real implementation, we would decrypt the winner's address
        // and mint the NFT to them. However, since FHE decryption must happen
        // off-chain, we'll emit an event for the frontend to handle the minting
        // or allow the winner to claim their NFT.
        
        // For now, we'll allow the winner to decrypt their address
        // and mint to the owner (who will then transfer to winner off-chain)
