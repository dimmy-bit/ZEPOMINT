...
/// @title ZepoMINTFHEAuctionSmartFinalizeFixed - Fixed smart finalization NFT auction
/// @author Mir Mohammed
/// @notice This contract implements smart finalization logic with better error handling:
/// - 0 bids: Finalize without winner
/// - 1 bid: Automatic winner
/// - 2+ bids: FHE-based winner determination with fallback
contract ZepoMINTFHEAuctionSmartFinalizeFixed is Ownable, ERC721 {
    ...
    /// @notice Smart finalization based on bid count with error handling
    function smartFinalize() external onlyOwner {
        ...
        if (bidCount == 0) {
            ...
            emit SmartFinalization("Single bid winner", bidCount);
            emit AuctionFinalized(winnerEncrypted, winningBidEncrypted, winningBidIndex);
        } else {
            // Multiple bids - use fallback approach
            _computeWinnerFallback();
        }
    }
    
    /// @notice Fallback winner computation for multiple bids
    function _computeWinnerFallback() private {
        ...
        // We'll set the index to 0 as a placeholder
        // In a real implementation, the frontend would determine the actual index
        winningBidIndex = 0;
        currentAuction.finalized = true;
        
        emit SmartFinalization("FHE-based winner determination", bids.length);
        emit AuctionFinalized(winner, highestBid, 0);
    }
